...
def init_pattern(cat):
    ...
    if cat.pelt.name in torties:
        ...
        elif cat.tortiebase == 'voisokoke':
            cat.tortiepattern = 'tortievoisokoke'
    ...
    if cat.pelt.name in torties and cat.pelt.colour is not None:
        if cat.pelt.colour in ["BLACK", "DARKBROWN", "GHOST", "VAMPIRE", "GRAVEL", "SLATEGREY", "CADETBLUE", "VIOLET", "DARKVIOLET", "BLUEVIOLET", "CHARCOAL"]:
...
def white_patches_inheritance(cat, parents: tuple):
    ...
    white_list = [little_white, mid_white, high_white, mostly_white, point_markings, ['FULLWHITE'], ['FULLBLACK'], ['FULLGINGER'], ['FULLBROWN']]

    weights = [0, 0, 0, 0, 0, 0, 0, 0, 0]  # Same order as white_list
    for p_ in par_whitepatches:
        if p_ in little_white:
            add_weights = (40, 20, 15, 5, 0, 0, 0, 0, 0)
        elif p_ in mid_white:
            add_weights = (10, 40, 15, 10, 0, 0, 0, 0, 0)
        elif p_ in high_white:
            add_weights = (15, 20, 40, 10, 0, 1, 1, 1, 1)
        elif p_ in mostly_white:
            add_weights = (5, 15, 20, 40, 0, 5, 5, 5, 5)
        elif p_ in point_markings:
            add_weights = (10, 10, 10, 10, 65, 5, 5, 5, 5)
        elif p_ == "FULLWHITE":
            add_weights = (0, 5, 15, 40, 0, 10, 10, 10, 10)
        else:
            add_weights = (0, 0, 0, 0, 0, 0, 0, 0, 0)
